...
class DidManagementApiControllerTest extends RestControllerTestBase {
    ...
    private RequestSpecification baseRequest() {
        return given()
                ...
                .baseUri("http://localhost:" + port + Versions.STABLE + "/participants/test-participant/dids")
                .when();
    }
    ...
    class RemoveEndpoint {
        ...
        void removeEndpoint_unauthorized403() {
            when(authService.authorize(any(), anyString(), anyString(), any())).thenReturn(ServiceResult.unauthorized("test message"));
            baseRequest()
                    .body(new DidRequestPayload(TEST_DID))
                    .delete("/%s/endpoints?serviceId=test-service-id".formatted(Base64.getUrlEncoder().encodeToString(TEST_DID.getBytes())))
                    .then()
                    .log().ifValidationFails()
                    .statusCode(403);
            verify(authService).authorize(any(), anyString(), anyString(), eq(DidResource.class));
    ...
    class Publish {
        ...
        void publish_unauthorized403() {
            when(authService.authorize(any(), anyString(), anyString(), any())).thenReturn(ServiceResult.unauthorized("test-msg"));
