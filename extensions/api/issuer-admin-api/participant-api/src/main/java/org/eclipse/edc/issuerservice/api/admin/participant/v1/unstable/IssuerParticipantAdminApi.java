...
public interface IssuerParticipantAdminApi {
    ...
    @Operation(description = "Adds a new participant.",
            ...
            requestBody = @RequestBody(content = @Content(schema = @Schema(implementation = Participant.class), mediaType = "application/json")),
            responses = {
    ...
    Response updateParticipant(Participant participant);
    ...
    @Operation(description = "Gets all participants for a certain query.",
            ...
            responses = {
                    @ApiResponse(responseCode = "200", description = "A list of participant metadata.",
                            content = @Content(array = @ArraySchema(schema = @Schema(implementation = Participant.class)), mediaType = "application/json")),
                    @ApiResponse(responseCode = "400", description = "Request body was malformed, or the request could not be processed",
                            content = @Content(array = @ArraySchema(schema = @Schema(implementation = ApiErrorDetail.class)), mediaType = "application/json")),
                    @ApiResponse(responseCode = "401", description = "The request could not be completed, because either the authentication was missing or was not valid.",
                            content = @Content(array = @ArraySchema(schema = @Schema(implementation = ApiErrorDetail.class)), mediaType = "application/json")),
    ...
    List<Participant> queryParticipants(QuerySpec querySpec);
}
