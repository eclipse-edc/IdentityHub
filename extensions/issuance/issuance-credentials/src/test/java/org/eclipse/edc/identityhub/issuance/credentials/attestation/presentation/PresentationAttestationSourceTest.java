...
class PresentationAttestationSourceTest {
    ...
    void verify_required() {
        ...
        var result = source.execute(new DefaultAttestationContext("participant", claims));
    ...
    void verify_notPresent() {
        var source = new PresentationAttestationSource("test", "testOut", true);
        assertThat(source.execute(new DefaultAttestationContext("participant", Map.of())).failed()).isTrue();
    }
}