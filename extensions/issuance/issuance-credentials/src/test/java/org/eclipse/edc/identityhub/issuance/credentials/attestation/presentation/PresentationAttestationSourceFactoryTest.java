...
class PresentationAttestationSourceFactoryTest {
    ...
    void verify_create() {
        ...
        var definition = new AttestationDefinition("123", "presentation", configuration);

        var source = factory.createSource(definition);

        var claims = Map.of("testCred", ClaimToken.Builder.newInstance().build());

        assertThat(source.execute(new DefaultAttestationContext("participant", claims)).succeeded()).isTrue();
    }
    ...
    void verify_create_optional() {
        ...
        assertThat(source.execute(new DefaultAttestationContext("participant", Map.of())).succeeded()).isTrue();
    }
