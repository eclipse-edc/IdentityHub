...
public class DcpPresentationFlowWithDockerTest {
    ...
    // the TCK seeds the credentials through the Storage API, which only accepts them from the Issuer the seeded
    // request was addressed to. The container binds the callback port 1:1, so the Holder reaches the TCK's DID
    // document at localhost; pinning the DID keeps both sides on the same value.
    ...
    @Test
    void runPresentationFlowTestsDocker(IdentityHub identityHub) throws InterruptedException {
